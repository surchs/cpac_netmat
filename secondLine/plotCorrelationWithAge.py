...
def dualHist(childCorr, adultCorr):
    '''
    Plot child and adult correlation together
    '''

    fig, (child, adult) = plt.subplots(1, 2, sharex=False, sharey=False)

    child.hist(childCorr)
    child.set_title('children')

    adult.hist(adultCorr)
    adult.set_title('adult')
...
def Main():
    ...
    # Define the outputs
    pathToCorrelationMatrix = '/home2/surchs/secondLine/correlation/abide/dos160/correlation_matrix_glob.txt'
    ...
    pathToPValueMatrix = '/home2/surchs/secondLine/correlation/abide/dos160/pvalue_correlation_matrix_glob.txt'
    pathToThresholdedMatrix = '/home2/surchs/secondLine/correlation/abide/dos160/thresholded_correlation_matrix_glob.txt'
    ...
    # Plot the distribution of correlation scores
    childMat, childP = correlateConnectomeWithAge(connectomeDict['child'],
                                                  ageDict['child'])
    adultMat, adultP = correlateConnectomeWithAge(connectomeDict['adult'],
                                                  ageDict['adult'])

    childCorr = prepareFDR(childMat)
    adultCorr = prepareFDR(adultMat)
    dualHist(childCorr, adultCorr)
    ...
    overallCorr = prepareFDR(correlationMatrix)

    plt.hist(overallCorr)
    plt.title('all')
    plt.show()
    plt.close()
